-- Estado do caderno, e só isso: quem tem e se está na mão. Nenhum Model, junta ou tween aqui —
-- o caderno é visual, cada cliente monta a própria réplica e o dono a dirige sem ida e volta.
-- O servidor existe para os outros clientes saberem em quem desenhar, e para quem entra depois
-- receber o retrato de quem já está em jogo.
local ManualService = {}
...
local function stateOf(player)
	...
	if not state then
		state = { equipped = false, inHand = false }
		states[player] = state
	end
	return state
end
